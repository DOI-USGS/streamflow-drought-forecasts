...
p1_targets <- list(
  ...
  tar_target(
    p1_conus_gages_parquet,
    download_s3_data(
      s3_bucket_name = p0_pipeline_bucket_name,
      aws_region = p0_aws_region,
      s3_filepath = "configuration/nwis_sw_gages.parquet", 
      outfile = "1_fetch/out/nwis_sw_gages.parquet"
    ),
    format = 'file'
  ),
  ...
  tar_target(
    ...
    download_forecast(
      ...
      aws_region = p0_aws_region,
      s3_bucket_name = p0_pipeline_bucket_name,
      outfile_template = "1_fetch/out/forecasts/fy25_operational_discrete_%sw_CalibrateBelow30_EnfQuant_PostprocMed_late_test_results.feather"
    ),
