...
const drawArea = function (group, { segment, dataKind, xScale, yScale, transitionLength }) {
  let areaElem
  ...
  areaElem = group.selectAll('path').data([segment.points], (d) => d[0].id)
  areaElem.join(
    (enter) =>
      enter
        .append('path')
        .attr('id', (d) => 'area-' + dataKind + '-' + segment.index + '-' + d[0].id)
        .attr('class', 'ts-area')
