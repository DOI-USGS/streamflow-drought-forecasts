...
const drawDiamond = function (group, { segment, dataKind, xScale, yScale, transitionLength }) {
  let rectElem
  // precompute key positioning parameters for each data point
  const xStarts = segment.points.map((d) =>
    xScale(d3TimeDay.offset(d.dateTime, -WIDTH_IN_DAYS / 2))
  )
  const rectWidthsHeights = segment.points.map(
    (d) => xScale(d3TimeDay.offset(d.dateTime, WIDTH_IN_DAYS)) - xScale(d.dateTime)
  )
  const yStarts = segment.points.map((d, index) => yScale(d.value) - rectWidthsHeights[index] / 2)
  rectElem = group.selectAll('rect').data(segment.points, (d) => d.id)
  rectElem.join(
    (enter) =>
      enter
        .append('rect')
        .attr('id', (d) => 'rect-' + dataKind + '-' + d.id)
