...
const drawRect = function (group, { segment, dataKind, xScale, yScale, transitionLength }) {
  let rectElem
  ...
  rectElem.join(
    (enter) =>
      enter
        ...
        .attr(
          'x',
          (d) => xScale(d3TimeDay.offset(d.dateTime, -WIDTH_IN_DAYS / 2)) + RECT_STROKE_WIDTH / 2
        )
        .attr('y', (d) => yScale(d.value_max))
        .attr(
          'width',
          (d) =>
            xScale(d3TimeDay.offset(d.dateTime, WIDTH_IN_DAYS)) -
            xScale(d.dateTime) -
            RECT_STROKE_WIDTH
        )
