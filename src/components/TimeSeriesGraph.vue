<template>
  <div id="timeseries-graph">
    <div id="timeseries-title-container">
      <p id="timeseries-title">
    ...
    >
      <div id="legend-content">
        <div>
          <span class="graph-legend-title slight-emph"> Drought category </span>
          <span
        ...
        <div>
          <span class="graph-legend-title slight-emph"> Observed streamflow </span>
          <span class="streamflow-legend-key-container"> <span />Last 90 days </span>
          <span class="streamflow-legend-box-container"> <span />Day before issue date </span>
        </div>
        <div>
          <span class="graph-legend-title slight-emph"> Predicted streamflow </span>
          <span class="forecast-legend-point-container"> <span />Median </span>
          <span class="forecast-legend-box-container"> <span />Uncertainty </span>
        </div>
      </div>
    </div>
    <div id="graph-control-container">
      ...
      />
      <GraphButtonDialog :show-prompt="true" />
    </div>
  </div>
</template>
...
const currentStreamflowDataset = computed(() => {
  return timeseriesDataStore.getFilteredDataset(
    selectedSite.value,
    'streamflow',
    'dt',
    globalDataStore.currentStreamflowDate
  )
})
...
const xScale = computed(() => {
  return timeScale(xDomain.value, layout.value.width)
})
...
const yScale = computed(() => {
  return waterDataScale(yDomain.value, layout.value.height, scaleKind.value == 'log', false)
})

const yTicks = computed(() => getWaterDataTicks(yDomain.value, scaleKind.value == 'log', false))
...
watch(selectedSite, (newValue) => {
  ...
  if (!storedDatasets.length > 0) {
    ...
    const fetchStreamflowDataPromise = timeseriesDataStore.fetchAndAddDatasets(
      newValue,
      'streamflow',
      ['result']
    )
    fetchDataPromises.push(fetchStreamflowDataPromise)
    const fetchForecastDataPromise = timeseriesDataStore.fetchAndAddDatasets(
      selectedSite.value,
      'forecasts',
      ['result']
    )
    fetchDataPromises.push(fetchForecastDataPromise)
    const fetchThresholdsDataPromise = timeseriesDataStore.fetchAndAddDatasets(
      selectedSite.value,
      'thresholds',
      ['result_min', 'result_max']
    )
    fetchDataPromises.push(fetchThresholdsDataPromise)
    const fetchOverlaysLowerDataPromise = timeseriesDataStore.fetchAndAddDatasets(
      selectedSite.value,
      'overlays_lower',
      ['result_min', 'result_max']
    )
    fetchDataPromises.push(fetchOverlaysLowerDataPromise)
    const fetchOverlaysUpperDataPromise = timeseriesDataStore.fetchAndAddDatasets(
      selectedSite.value,
      'overlays_upper',
      ['result_min', 'result_max']
    )
    fetchDataPromises.push(fetchOverlaysUpperDataPromise)
    const fetchStreamflowDroughtsDataPromise = timeseriesDataStore.fetchAndAddDatasets(
      selectedSite.value,
      'streamflow_droughts',
      []
    )
