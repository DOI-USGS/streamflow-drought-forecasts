...
const transform = computed(
  () => `translate(${props.layout.margin.left}, ${props.layout.height + props.layout.margin.top})`
)
...
const streamflowDroughtIntervals = computed(() => {
  return props.streamflowDroughtsData.values.map((drought) => {
    ...
    return {
      ...
      id: `bar-rect-drought-${drought.StaID}-${drought.start}`
    }
  })
})
...
const forecastDroughtPoints = computed(() => {
  return props.forecastDroughtsData.values.map((drought) => {
    ...
    const radius =
      (props.xScale(d3TimeDay.offset(droughtDate, WIDTH_IN_DAYS)) - props.xScale(droughtDate)) / 2
    return {
      cx: cx,
      r: radius,
      class: `bar-point point-${drought.drought_cat}`,
      id: `bar-circle-forecast-${drought.StaID}-${drought.dt}`
    }
  })
})
const backgroundForecastDroughtPoints = computed(() => {
  return props.forecastDroughtsData.values.map((drought) => {
    ...
    return {
      ...
      id: `bar-circle-background-forecast-${drought.StaID}-${drought.dt}`
    }
  })
})
...
watchEffect(() => {
  if (streamflowDroughtsGroup.value) {
    ...
    select(streamflowDroughtsGroup.value)
      .selectAll('.bar-point-background')
      .style('stroke-width', '1px')
    select(streamflowDroughtsGroup.value)
      .selectAll('.bar-point')
      .style('stroke', 'var(--grey_6_1)')
      .on('click', (event) => {
        const elementDate = event.target.id.slice(29)
        const elementWeek =
    ...
    const currentBackgroundPoint = select(streamflowDroughtsGroup.value).select(
      `#bar-circle-background-forecast-${globalDataStore.selectedSite}-${globalDataStore.selectedDate}`
    )
    if (currentBackgroundPoint.node()) {
      ...
      if (currentBackgroundPointClassList[1].includes('none')) {
        currentBackgroundPoint.style('stroke-width', '4.5px')
      } else {
        currentBackgroundPoint.style('stroke-width', '5px')
      }
    }
    const currentPoint = select(streamflowDroughtsGroup.value).select(
      `#bar-circle-forecast-${globalDataStore.selectedSite}-${globalDataStore.selectedDate}`
    )
    if (currentPoint.node()) {
      const currentPointClassList = currentPoint.node().classList
      if (currentPointClassList[1].includes('none')) {
        currentPoint.style('stroke', 'var(--grey_2_1)')
      } else {
        currentPoint.style('stroke', 'var(--white-soft)')
      }
    }
  }
})
